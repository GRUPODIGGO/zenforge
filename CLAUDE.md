...
- `CNAME` na raiz do repo com o valor `zenforge.testaapp.live` — é isso que diz ao GitHub Pages qual domínio customizado servir.
- Imagens (logos do Grupo Diggo/TestaApp/BuildSpeed, foto do fundador) estão **embutidas como base64** direto no HTML — não há pasta de assets separada. Isso deixa o arquivo pesado (~1.9 MB) mas o site continua 100% self-contained.
...
4. Pra confirmar que subiu: `curl http://zenforge.testaapp.live` (HTTPS pode falhar por período — ver nota abaixo) e conferir o conteúdo esperado.
